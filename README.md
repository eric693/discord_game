...
打開 `discord_bot_fixed.py`，找到第 27-30 行：

```python
ADMIN_USER_IDS = [
    # 在這裡填入你的 Discord 用戶 ID
]
```
...
```python
ADMIN_USER_IDS = [
    123456789012345678,  # 替換成你的實際 ID
]
```

**重要：** 管理員會自動跳過驗證檢查，可以直接使用所有指令。
...
#### 自訂序號池（手動輸入序號）
```
/add_custom_serials code:STEAM2026 item_name:Steam激活碼 serials:KEY1AAA,KEY2BBB,KEY3CCC duration:永久
```

**注意：** 序號格式為20碼純文字（無短橫線、無空格）
...
**管理員權限：**
- 在 `ADMIN_USER_IDS` 列表中的用戶
- 或擁有 Discord「管理員」權限的用戶
- 管理員自動跳過驗證檢查，可使用所有指令
...
### Q: 為什麼指令無法使用？顯示 CheckFailure 錯誤？

**A**: 這通常是權限問題。請檢查：
1. **管理員用戶**：確認你的 Discord ID 已加入 `ADMIN_USER_IDS` 列表並重啟機器人
2. **一般用戶**：確認是否已獲得驗證身分組
3. 機器人是否有足夠的權限
4. 指令是否已成功同步（查看啟動訊息）

**快速解決：**
- 獲取你的 Discord ID（開啟開發者模式 → 右鍵點擊名字 → 複製 ID）
- 將 ID 加入程式碼中的 `ADMIN_USER_IDS` 列表
- 重啟機器人

### Q: 如何新增更多管理員？

**A**: 在程式碼中的 `ADMIN_USER_IDS` 列表新增 Discord ID：

```python
ADMIN_USER_IDS = [
    123456789012345678,  # 管理員 A
    234567890123456789,  # 管理員 B
    345678901234567890,  # 管理員 C
]
```
...
### 3. 權限管理
- 僅給予機器人必要的權限
- 定期審查管理員名單（`ADMIN_USER_IDS`）
- 限制敏感指令的使用
- 不要將管理員 ID 公開到 GitHub
- 管理員帳號應使用強密碼和雙重驗證
